...
const AddTodo = (props) => {
    ...
    const createTodo = async () => {
        ...
        try{
            const res = await axios.post('http://localhost:8005/todos', newTodo)
            if(res.status === 201){
               props.getTodos()
        ...
        <Dialog
            ...
            content={({ hide }) => (
                <div className="flex flex-column px-8 py-5 gap-4" style={{ borderRadius: '12px', backgroundImage: 'radial-gradient(circle at left top, var(--primary-400), var(--primary-700))' }}>
                    ...
                    <Button label="add" onClick={(e) => { createTodo (title,tags); hide(e) }} text className="p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10"></Button>
